...
const cases = [
  ...
  {
    name: 'OpenAI 路径 (/v1/chat/completions) 注入 system 消息',
    check: () => {
      ...
      const out = JSON.parse(processRequestBody(JSON.stringify(body), '/v1/chat/completions'));
      return out.messages[0].role === 'system' && out.messages[0].content === 'You are a helpful software engineer assistant.';
    }
  },
  {
    name: 'Anthropic 路径 (/v1/messages) 无 system 用顶层 system 字段(不插非法消息)',
    check: () => {
      const body = { model: 'deepseek-v4-pro', messages: [{ role: 'user', content: 'hi' }], tools: allTools };
      const out = JSON.parse(processRequestBody(JSON.stringify(body), '/v1/messages'));
