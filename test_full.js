...
function post(pathName, bodyStr, headers) {
  return new Promise((resolve, reject) => {
    const req = http.request({ hostname: '127.0.0.1', port: 20129, path: pathName, method: 'POST', headers: { 'content-type': 'application/json', ...(headers || {}) }, timeout: 8000 }, (res) => {
      let data = '';
...
function get(pathName) {
  return new Promise((resolve, reject) => {
    const req = http.request({ hostname: '127.0.0.1', port: 20129, path: pathName, method: 'GET', timeout: 5000 }, (res) => {
      let data = '';
      res.on('data', c => data += c);
...
async function main() {
  try {
    ...
    check('A25 防自环后代理仍存活', await state.isProxyRunning(20129));
    ...
    const sse = await new Promise((resolve, reject) => {
      const req = http.request({ hostname: '127.0.0.1', port: 20129, path: '/v1/messages/stream', method: 'POST', headers: { 'content-type': 'application/json' }, timeout: 8000 }, (res) => {
        let data = '';
        res.on('data', c => data += c);
        res.on('end', () => resolve({ status: res.statusCode, body: data, headers: res.headers }));
      });
    ...
    check('B3 空闲后 daemon 自毁退出', !(await state.isProxyRunning(20129)));
