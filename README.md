...
#### 🌟 方式 A：一键任务模式（最推荐）
直接在聊天框输入 `/we-need-ds` 并附带你的任务：
```
/we-need-ds 帮我分析当前项目的架构并制定一份迁移到 ESM 模块的方案
```
插件会自动开启环境拦截，并带入你的任务首轮锁定 "We need..." 深度推理！

#### 📐 方式 B：深度规划专家子代理
遇到极其庞大的工程任务，想先让模型只做调研、不做修改：
```
/we-need-ds:plan 规划重构整个数据库连接池架构
```
该命令会唤起专属子代理 `we-need-planner`，在只读环境下产出标准 Markdown 执行蓝图供你审核。

#### 🔧 方式 C：状态与维护命令
* `/we-need-ds:status`：查看当前代理守护进程运行状态与拦截开关。
* `/we-need-ds:on`：手动强制开启拦截。
* `/we-need-ds:off`：手动关闭拦截并即时还原端点配置。
