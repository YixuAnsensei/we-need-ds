...
**失败即还原（防死锁）**：任何恢复路径（`on` / `boot` / 钩子）如果拉起 daemon 失败，都会把仍指向代理端口的 provider **自动还原为真实上游直连**——绝不留"端点指向死代理、用户无法使用、恢复命令也发不出"的死锁状态。`status` / `doctor` 若检测到 provider 指向代理端口但 daemon 未运行，会打印醒目告警并给出恢复命令。
