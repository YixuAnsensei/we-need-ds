...
function parseFlag(name) {
  ...
  if (val === undefined || val.startsWith('--')) {
    console.log(`[we-need-ds] 参数错误：--${name} 缺少取值（用法：on --provider <id 或名称>，可先用 list 查看清单）`);
    process.exit(1);
  }
  return val;
}

function daemonCtl(port, action, extra) {
  return new Promise((resolve) => {
    ...
    req.end(JSON.stringify({ action, ...(extra || {}) }));
  });
}
...
async function main() {
  if (command === 'boot') {
    ...
    if (st.enabled) {
      ...
      if (running) {
        ...
        const kill = await state.killDaemonOnPort(config.port);
        if (kill.foreign) {
          console.log(`[we-need-ds] boot: 端口 :${config.port} 被无关进程(PID ${kill.pid})占用，已跳过清理（非本插件 daemon，不误杀）`);
        }
      }
  ...
  if (command === 'restart') {
    ...
    const kill = await state.killDaemonOnPort(config.port);
    if (kill.killed) {
      console.log(`[we-need-ds] restart: 停止旧 daemon (PID ${kill.pid})${kill.released ? '' : '（端口释放超时，仍尝试拉起）'}`);
    } else if (kill.foreign) {
      console.log(`[we-need-ds] restart 中止：端口 :${config.port} 被无关进程(PID ${kill.pid})占用，本插件不误杀无关进程。请改 config.json 端口或释放该端口后重试。`);
      process.exit(1);
    } else {
  ...
  if (command === 'list') {
    const lp = state.listProviders(config);
    if (!lp.ok) {
      console.log(`[we-need-ds] list 失败：${lp.reason}`);
      process.exit(1);
    }
    console.log(`\n======================================================`);
    console.log(` [we-need-ds] Provider 清单 (共 ${lp.providers.length} 个，含 DS 模型的排在前面)`);
    console.log(`======================================================`);
    lp.providers.forEach((p, idx) => {
      const marks = [];
      if (p.hasTargetModels) marks.push('🎯含DS模型');
      if (p.isDefault) marks.push('⭐当前默认');
      if (p.isHooked) marks.push(`🔌代理中`);
      console.log(`  [${idx + 1}] ${p.name}  ${marks.length ? '(' + marks.join(' ') + ')' : ''}`);
      console.log(`      id: ${p.id}`);
      console.log(`      baseUrl: ${p.baseUrl}`);
      if (p.models) console.log(`      models: ${p.models}`);
    });
    console.log(`\n接管指定 provider：node lib/ctl.js on --provider <id 或名称>`);
    console.log(`说明：接管非默认 provider 时会自动把 activeId 切到它（cc-haha 按 activeId 路由，必须同步）。\n`);
    return;
  }
  ...
  if (command === 'on') {
    ...
    let r = await daemonCtl(config.port, 'on', providerArg ? { providerId: providerArg } : null);
    if (r.reachable) {
    ...
    if (r.ok) {
      ...
      console.log(`======================================================`);
      console.log(`⚡ DSH 极简模拟待命：判定轮(新任务)自动切换极简环境(剥 system-reminder + 注入 thinking)，执行轮(工具续跑)全量放行 (we-need-ds:v5.2)`);
      if (r.mode === 'cc-haha') {
        if (r.activeHooked) {
          console.log(`已接管 Provider「${r.activeHooked}」→ :${config.port}，并建直连副本「${r.copyName}」(逃生口，可随时切回直连)`);
          if (r.activeSwitched) {
            console.log(`ℹ️ 已把 cc-haha 默认 provider (activeId) 切换为「${r.activeHooked}」——sidecar 按 activeId 路由，接管非默认 provider 必须同步切换。`);
          }
        } else if (r.note) {
          console.log(`ℹ️ ${r.note}`);
        }
      }
    ...
    if (r.ok) {
      ...
      if (r.removedCopies && r.removedCopies > 0) {
        console.log(`\n已移除 ${r.removedCopies} 个临时直连副本（本体确认还原后才清除）。`);
      }
      if (r.unrestorableList && r.unrestorableList.length > 0) {
        console.log(`\n⚠️ 警告：以下 ${r.unrestorableList.length} 个 Provider 仍指向代理端口，但账本与副本均无真实上游记录，无法自动还原：`);
    ...
    if (hasCc) {
      try {
        ...
        const activeProv = (data.providers || []).find(p => p.id === data.activeId && !state.isCopyProvider(p));
        console.log(`3. 当前默认 Provider: ${activeProv ? activeProv.name : '未知'} (${activeProv ? activeProv.baseUrl : ''})`);

        const st = state.readState();
        const hookedCount = st.enabled && st.providers ? Object.keys(st.providers).length : 0;
        console.log(`4. 拦截接管状态: ${st.enabled ? `✅ 已开启 (单服务商接管 ${hookedCount} 个，仅默认 DS provider 走代理)` : '⚪ 已关闭 (直连上游)'}`);
        console.log(`5. 极简模式: ⚡ v5.2 常态生效 (每个判定轮=DSH 极简环境+剥reminder+注入thinking；每个执行轮=全量工具)`);

        if (st.enabled && st.providers && Object.keys(st.providers).length > 0) {
          const drift = [];
          for (const [lid, rec] of Object.entries(st.providers)) {
            const prov = (data.providers || []).find(p => p.id === lid && !state.isCopyProvider(p));
            if (!prov) {
              drift.push({ name: (rec && rec.name) || lid, why: '已从 providers 池消失（被删除或重建导致 id 漂移）' });
            } else if (!prov.baseUrl || !prov.baseUrl.includes(`:${config.port}`)) {
              drift.push({ name: prov.name || lid, why: `baseUrl 已不指向代理(:${config.port})，当前=${prov.baseUrl || '空'}` });
            }
          }
          const anyOnProxy = (data.providers || []).some(p => !state.isCopyProvider(p) && p.baseUrl && p.baseUrl.includes(`:${config.port}`));
          if (drift.length || !anyOnProxy) {
            console.log(`\n⚠️ 接管漂移告警：账本记录处于接管态，但接管已悬空 → "We need" 裁剪可能已静默失效！`);
            drift.forEach(d => console.log(`   - 「${d.name}」: ${d.why}`));
            if (!drift.length && !anyOnProxy) console.log(`   - 没有任何 provider 当前指向代理端口 :${config.port}`);
            console.log(`   恢复：在 cc-haha 选好目标 DS 服务商后执行 /we-need-ds:on（或终端 node lib/ctl.js on），重新接管即恢复裁剪`);
          }
        }

        const copies = (data.providers || []).filter(p => state.isCopyProvider(p));
        if (copies.length > 0) {
          console.log(`\n★ 直连副本 (逃生口，代理死掉时可切到它直连恢复)：`);
          copies.forEach(c => console.log(`   - ${c.name}  --->  ${c.baseUrl}`));
        }

        let matched = [];
        for (const prov of (data.providers || [])) {
          if (state.isCopyProvider(prov)) continue;
    ...
    console.log(`极简模式          : ⚡ v5.2 常态生效 (判定轮=DSH 极简+剥reminder+注入thinking；执行轮=全量放行)`);

    if (st.enabled && hookedProviders.length > 0) {
      console.log(`\n当前被代理接管的 Provider (${hookedProviders.length} 个，单服务商模式):`);
      hookedProviders.forEach((p, idx) => {
        console.log(`  ${idx + 1}. ${p.name} (原始上游: ${p.originalUrl})`);
      });
      try {
        const pdata = JSON.parse(require('fs').readFileSync(state.PROVIDERS_PATH, 'utf8'));
        const copies = (pdata.providers || []).filter(p => state.isCopyProvider(p));
        copies.forEach(c => console.log(`  ★ 直连副本: ${c.name} ---> ${c.baseUrl} (代理死掉时切到它即可直连恢复)`));
      } catch (e) {}
    } else {
