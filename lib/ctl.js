...
const autostart = require(path.join(__dirname, 'autostart.js'));
...
async function main() {
  if (command === 'boot') {
    ...
    if (st.enabled) {
      ...
      if (!alive) {
        console.log(`[we-need-ds] boot: 拦截处于开启状态，但 daemon 拉起失败(:${config.port})，providers 保持原样待人工核对`);
        return;
      }
  ...
  if (command === 'on') {
    ...
    if (r.ok) {
      ...
      const as = autostart.install();
      if (as.ok) {
        console.log(`🔌 开机自愈已自动启用：登录时静默跑 ctl boot，重启后代理自动复活并按账本接管（无需管理员权限）`);
      } else {
        console.log(`⚠️ 开机自愈启用失败（不影响本次拦截）：${as.error}`);
      }
      console.log(`======================================================\n`);
      return;
    } else {
    ...
    if (sub === 'install') {
      const a = autostart.install();
      console.log(a.ok ? `[we-need-ds] 开机自愈已启用: ${a.path}` : `[we-need-ds] 启用失败: ${a.error}`);
    } else if (sub === 'uninstall') {
      const a = autostart.uninstall();
      console.log(a.ok ? (a.removed ? `[we-need-ds] 开机自愈已移除: ${a.removed}` : `[we-need-ds] 开机自愈本就未启用`) : `[we-need-ds] 移除失败: ${a.error}`);
    } else {
      const s = autostart.status();
      console.log(`[we-need-ds] 开机自愈: ${s.installed ? '✅ 已启用' : '⚪ 未启用'}`);
      console.log(`  启动项: ${s.path}`);
      console.log(`  开机时静默执行 ctl boot，按账本 enabled 状态自愈（开启则复活+接管，关闭则清理残留）`);
    }
    return;
  } else if (command === 'off') {
