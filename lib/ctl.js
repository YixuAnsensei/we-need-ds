...
async function main() {
  ...
  if (command === 'on' || command === 'arm') {
    ...
    if (r.ok) {
      ...
      console.log(`======================================================`);
      console.log(`⚡ 武装窗口待命 (${config.armWindowMinutes || 20} 分钟内首个新任务轮触发裁剪，成功一次后关闭；失败重试不消耗) (we-need-ds:armed)`);
      if (r.mode === 'cc-haha') {
    ...
    if (hasCc) {
      try {
        ...
        console.log(`5. 武装窗口: ${state.isArmActive(st) ? `⚡ 待命中 (至 ${new Date(Date.parse(st.forceArmedAt) + (st.armWindowMinutes || 20) * 60000).toLocaleTimeString('zh-CN')}，首个新任务轮消耗)` : '⚪ 未生效 (首轮自动判定)'}`);
    ...
    console.log(`武装窗口          : ${state.isArmActive(st) ? '⚡ 待命中 (首个新任务轮消耗)' : '⚪ 未生效 (首轮自动判定)'}`);
