...
async function main() {
  if (command === 'boot') {
    ...
    if (st.enabled) {
      ...
      console.log(`[we-need-ds] boot: 拦截处于开启状态，daemon ${running ? '已在运行' : '已重新拉起'}${hooked ? `，修复孤儿 provider ${hooked} 个` : ''}`);
    } else {
      const r = state.recoverOrphans(config);
      const hooked = r.restoredList ? r.restoredList.length : 0;
      if (running) {
        ...
        const pid = await new Promise((resolve) => {
          exec('netstat -ano | findstr ":20129" | findstr "LISTENING"', (err, stdout) => {
  ...
  if (command === 'on' || command === 'arm') {
    ...
    let r = await daemonCtl(config.port, command === 'on' ? 'on' : 'arm');
    if (!r.reachable) {
      r = state.enableInterception(config, { arm: command === 'on' });
      if (r.ok && command === 'on') state.armForceWindow(config);
      r.fallback = true;
    }
    if (r.ok) {
      console.log(`\n======================================================`);
      ...
      console.log(`======================================================`);
      console.log(`⚡ 武装窗口待命 (${config.armWindowMinutes || 20} 分钟；目标轮全部裁剪，执行链结束后自动关闭；失败重试不消耗) (we-need-ds:armed)`);
      if (r.fallback) {
        console.log(`(daemon 控制端点不可达，已走文件模式)`);
      }
      if (r.mode === 'cc-haha' || (r.body && r.body.mode === 'cc-haha')) {
    ...
    if (hasCc) {
      try {
        ...
        console.log(`5. 武装窗口: ${state.isArmActive(st) ? `⚡ 待命中 (至 ${new Date(Date.parse(st.forceArmedAt) + (st.armWindowMinutes || 20) * 60000).toLocaleTimeString('zh-CN')}，目标轮全部裁剪，执行链结束后自动关闭)` : '⚪ 未生效 (首轮自动判定)'}`);
    ...
    console.log(`武装窗口          : ${state.isArmActive(st) ? '⚡ 待命中 (目标轮全部裁剪，执行链结束后自动关闭)' : '⚪ 未生效 (首轮自动判定)'}`);
