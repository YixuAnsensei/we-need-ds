...
async function main() {
  ...
  if (command === 'on' || command === 'arm') {
    ...
    if (r.ok) {
      ...
      console.log(`======================================================`);
      console.log(`⚡ 武装窗口已开启 (${config.armWindowMinutes || 30} 分钟内每轮均裁剪+还原人设) (we-need-ds:armed)`);
      if (r.mode === 'cc-haha') {
    ...
    if (hasCc) {
      try {
        ...
        console.log(`5. 武装窗口: ${state.isArmActive(st) ? `⚡ 生效中 (至 ${new Date(Date.parse(st.forceArmedAt) + (st.armWindowMinutes || 30) * 60000).toLocaleTimeString('zh-CN')}，窗口内每轮裁剪)` : '⚪ 未生效 (首轮自动判定)'}`);
    ...
    console.log(`武装窗口          : ${state.isArmActive(st) ? '⚡ 生效中 (每轮裁剪)' : '⚪ 未生效 (首轮自动判定)'}`);
