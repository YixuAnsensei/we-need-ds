...
function armForceNextTurn() {
  const st = readState();
  st.forceNextTurn = true;
  st.forceArmedAt = new Date().toISOString();
  writeState(st);
  log('forceNextTurn armed: next prompt will trigger 4-tool bootstrap');
}

function consumeForceNextTurn() {
  const st = readState();
  if (st.forceNextTurn) {
    st.forceNextTurn = false;
    writeState(st);
    log('forceNextTurn consumed');
    return true;
  }
  return false;
}
...
function enableInterception(config, options = {}) {
  ...
  writeProviders(data);
  log(`interception ON (cc-haha mode): ${interceptedList.length} providers hooked (forceNextTurn=${shouldArm})`);
  return {
    ok: true,
    mode: 'cc-haha',
    proxyUrl,
...
function disableInterception(config) {
  ...
  if (!hasCcHahaProviders()) {
    writeState({ enabled: false, providers: {}, keyMap: {}, defaultUpstream: null, forceNextTurn: false });
...
function readState() {
  ...
  return { enabled: false, providers: {}, keyMap: {}, defaultUpstream: null, forceNextTurn: false };
}
...
module.exports = {
  ...
  armForceNextTurn, consumeForceNextTurn, isSelfProxyUrl,
