...
function enableInterception(config, opts) {
  ...
  if (providerId) {
    ...
    if (!activeProv) {
      writeProviders(data);
      return { ok: false, reason: `未找到 id 为「${providerId}」的 provider（可用 list-providers 查看）` };
    }
    if (!providerHasTargetModels(activeProv, config.targetModels)) {
      writeProviders(data);
      return { ok: false, reason: `provider「${activeProv.name}」的 models 字段未声明 DeepSeek Pro 模型，接管无意义（裁剪不适用）。请从 list-providers 里选一个含 DS 模型的。` };
    }
