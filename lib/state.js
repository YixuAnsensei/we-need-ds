...
function loadConfig() {
  ...
  if (config.armWindowMinutes === undefined) config.armWindowMinutes = 20;
  return config;
}
...
function armForceWindow(config) {
  const st = readState();
  st.forceArmedAt = new Date().toISOString();
  st.armWindowMinutes = (config && config.armWindowMinutes) || 20;
  st.forceNextTurn = false;
  writeState(st);
  log(`arm window opened: ${st.armWindowMinutes} min, trimmed turns consume it on success`);
}

function isArmActive(st) {
  if (!st || !st.forceArmedAt) return false;
  const start = Date.parse(st.forceArmedAt);
  if (isNaN(start)) return false;
  const win = (st.armWindowMinutes || 20) * 60 * 1000;
  return Date.now() - start < win;
}

function markChainSeen() {
  const st = readState();
  if (!st || !st.forceArmedAt) return;
  st.chainSeen = true;
  writeState(st);
}

function isChainSeen(st) {
  return !!(st && st.chainSeen);
}

function consumeArmWindow() {
  const st = readState();
  if (!st || !st.forceArmedAt) return false;
  st.forceArmedAt = null;
  st.chainSeen = false;
  writeState(st);
  log('arm window consumed: execution chain ended');
  return true;
}

function isSelfProxyUrl(url, port) {
  if (!url) return false;
  return url.includes(`:${port}`) || url.includes('127.0.0.1:20129') || url.includes('localhost:20129');
}

function enableInterception(config, options = {}) {
  const proxyUrl = `http://127.0.0.1:${config.port}`;
  const shouldArm = options.arm !== false;
  ...
  if (!hasCcHahaProviders()) {
    writeState({
      ...
      forceArmedAt: shouldArm ? new Date().toISOString() : null,
      armWindowMinutes: (config.armWindowMinutes || 20),
      ts: new Date().toISOString()
    });
    log(`interception ON (pure Claude Code mode): listening on ${proxyUrl} (armWindow=${shouldArm ? config.armWindowMinutes || 30 : 0}min)`);
  ...
  writeProviders(data);
  log(`interception ON (cc-haha mode): ${interceptedList.length} providers hooked (armWindow=${shouldArm ? config.armWindowMinutes || 30 : 0}min)`);
  return {
    ok: true,
    mode: 'cc-haha',
    proxyUrl,
...
function disableInterception(config) {
  ...
  if (!hasCcHahaProviders()) {
    writeState({ enabled: false, providers: {}, keyMap: {}, defaultUpstream: null, forceArmedAt: null, forceNextTurn: false });
  ...
  if (restoredList.length === 0) {
    writeState({ ...state, enabled: false, forceArmedAt: null, forceNextTurn: false, ts: new Date().toISOString() });
...
function readState() {
  ...
  return { enabled: false, providers: {}, keyMap: {}, defaultUpstream: null, forceArmedAt: null, forceNextTurn: false };
}
...
module.exports = {
  ...
  armForceWindow, isArmActive, isSelfProxyUrl, consumeArmWindow, markChainSeen, isChainSeen,
