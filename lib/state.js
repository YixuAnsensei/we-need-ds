...
function isProxiedUrl(url, ports) {
  ...
  for (const port of ports) {
    if (url.includes(`127.0.0.1:${port}`) || url.includes(`localhost:${port}`)) return true;
  }
  return false;
}
...
function enableInterception(config) {
  ...
  const activeProv = (data.providers || []).find(p => p.id === data.activeId && !isCopyProvider(p));
  ...
  if (!providerHasTargetModels(activeProv, config.targetModels)) {
    ...
    return { ok: true, mode: 'cc-haha', proxyUrl, interceptedList: [], activeHooked: null, note: `默认 provider「${activeProv.name}」非 DeepSeek Pro，未接管；要触发裁剪请把 DS provider 设为默认` };
  }
  ...
  log(`interception ON (cc-haha single-provider): 接管「${activeProv.name}」→ :${config.port}，并建直连副本「${copy.name}」(原始 ${realOriginal})`);
  return {
    ok: true,
    mode: 'cc-haha',
    proxyUrl,
