...
function loadConfig() {
  ...
  if (config.armWindowMinutes === undefined) config.armWindowMinutes = 30;
  return config;
}
...
function armForceWindow(config) {
  ...
  st.armWindowMinutes = (config && config.armWindowMinutes) || 30;
  st.forceNextTurn = false;
  writeState(st);
  log(`arm window opened: ${st.armWindowMinutes} min, every turn in window gets trimmed`);
}
...
function isArmActive(st) {
  ...
  const win = (st.armWindowMinutes || 30) * 60 * 1000;
...
function enableInterception(config, options = {}) {
  ...
  if (!hasCcHahaProviders()) {
    writeState({
      ...
      armWindowMinutes: (config.armWindowMinutes || 30),
...
module.exports = {
  ...
  armForceWindow, isArmActive, isSelfProxyUrl,
