...
function isSelfProxyUrl(url, port) {
  if (!url) return false;
  return url.includes(`:${port}`) || url.includes(`:${port}/`);
}
...
function enableInterception(config) {
  ...
  const prevPort = (() => {
    try { return currentState.proxyUrl ? new URL(currentState.proxyUrl).port : null; } catch (e) { return null; }
  })();
  if (prevPort && prevPort !== String(config.port)) {
    for (const prov of data.providers) {
      if (!prov.baseUrl) continue;
      const isStaleProxy = prov.baseUrl.includes(`127.0.0.1:${prevPort}`) || prov.baseUrl.includes(`localhost:${prevPort}`);
      if (isStaleProxy && savedProviders[prov.id] && savedProviders[prov.id].originalUrl && !isSelfProxyUrl(savedProviders[prov.id].originalUrl, config.port)) {
        prov.baseUrl = savedProviders[prov.id].originalUrl;
        log(`interception ON: provider ${prov.name || prov.id} 指向旧代理端口 ${prevPort}，已先还原为真实上游再按新端口 ${config.port} 接管`);
      }
    }
  }
...
module.exports = {
  ...
  enableInterception, disableInterception, recoverOrphans, readState, writeState, updateState,
  isSelfProxyUrl,
