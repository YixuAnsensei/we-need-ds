...
function queryProcessImage(pid) {
...
async function killDaemonOnPort(port, waitMs) {
  ...
  if (!pid) return { killed: false, foreign: false, pid: null, released: true };
  const isMine = await isProxyRunning(port);
  if (!isMine) {
    const image = queryProcessImage(pid);
    const looksLikeNode = image === 'node.exe' || image === 'node';
    if (!looksLikeNode) {
      log(`killDaemonOnPort :${port} pid=${pid} 判定为无关进程（health-check 不通且映像=${image || '未知'}），拒绝误杀`);
      return { killed: false, foreign: true, pid, released: false };
    }
  }
  ...
  return { killed: true, foreign: false, pid, released };
}
